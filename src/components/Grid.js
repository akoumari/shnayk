...
const Grid = () => {
  ...
  const [highScore, setHighScore] = useState(Number(localStorage.getItem("score")));
      ...
      ...grid.map((row) => {
        let inner = row.map((col) => {
          return {
            ...col,
            hasSnk:
              snake.body.filter((part) => part.x == col.x && part.y == col.y)
                ?.length >= 1,
        ...
        {grid.map((row) => {
          return row.map((col) => {
            return (
              <div
                key={`(${col.x}, ${col.y})`}
                className={`grid-item ${col.hasFood ? "food" : ""} ${
  ...
  return (
    <div className={``}>
      ...
      <div>
        ...
        {snake.gameOver ? "GAME OVER" : `Score: ${score}`}
          </div> 
      <div>
